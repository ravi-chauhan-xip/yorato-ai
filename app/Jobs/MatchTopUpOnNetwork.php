...
class MatchTopUpOnNetwork implements ShouldQueue
{
    use CoinTrait;
    ...
    public function handle()
    {
        DB::transaction(function () {
            //            if ($this->member->left_id && $this->member->right_id &&
            ...
            //            ) {
            while (
                ($this->member->left_power >= settings('min_matching') && $this->member->right_power >= settings('min_matching'))
            ) {
                $paidBinaryCount = $this->member->binaryMatches()->count();
                // Match any PV of this is not the first binary income
                if ($paidBinaryCount >= 0) {
                    if (($this->member->left_power >= settings('min_matching') && $this->member->right_power >= settings('min_matching'))) {
                        if ($this->member->left_power >= $this->member->right_power) {
                            $pvMatch = $this->member->right_power;
                        } else {
                            $pvMatch = $this->member->left_power;
                        }

                        $this->member->left_power -= $pvMatch;
                        $left = $pvMatch;
                        $this->member->right_power -= $pvMatch;
                        $right = $pvMatch;
                    } else {
                        break;
                    }
    ...
    public function createBinaryTransaction(Member &$member, $left, $right, $pvMatch)
    {
        DB::transaction(function () use ($pvMatch, $right, $left, $member) {
            $lastMatchingBonusIncome = $member->binaryMatches()->orderBy('id', 'desc')->first();

            $cappingReached = false;
            $amount = $pvMatch * 15 / 100;
            $comment = 'Team matching wallet income of '.toHumanReadable($left).':'.toHumanReadable($right).' '.env('APP_CURRENCY_USDT').' matching through Top up';
            ...
            $income = $member->binaryMatches()->create([
                'amount' => $amount,
                'admin_charge' => 0,
                'tds' => 0,
                'total' => $amount,
                ...
                'left_total_bv' => $member->left_bv,
                'right_total_bv' => $member->right_bv,
                'left_forward_bv' => $member->left_power,
                'right_forward_bv' => $member->right_power,
                'left_new_bv' => $member->left_bv - optional($lastMatchingBonusIncome)->left_total_bv,
                'right_new_bv' => $member->right_bv - optional($lastMatchingBonusIncome)->right_total_bv,
