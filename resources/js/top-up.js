...
createApp({
    setup() {
        ...
        const getPackage = async () => {
            await axios.get('/user/get-packages')
                .then(function (response) {
                    packages.value = response.data.packages
                    currentPackage.value = response.data.currentPackage
                })
        }

        const selectPackage = (packageId) => {
            if (packageId) {
                selectedPackage.value = packages.value.find(item => item.id === packageId);
                if (currentPackage.value) {
                    amount.value = selectedPackage.value.amount - currentPackage.value.amount
                } else {
                    amount.value = selectedPackage.value.amount
                }
            } else {
        ...
        const stakeCoinValidation = async () => {
            ...
            await axios.post('/user/top-up/store-validation', {
        ...
        const stakeCoin = async (e) => {
            ...
            if (!isStakeCoinValidationFail.value) {
                ...
                try {
                    ...
                    await axios.post('/user/top-up/store', {
                        'walletAddress': walletAddress.value,
                        'package_id': package_id.value,
