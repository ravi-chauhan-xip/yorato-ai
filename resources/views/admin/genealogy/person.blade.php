<div class="person mb-2">
    @if($member)
        <div tabindex="0" data-bs-html="true"
             ...
             data-bs-content="@include('admin.genealogy.popover', ['member' => $member])">
            @if(Agent::isMobile())
            ...
            @else
                <a href="{{ route('admin.genealogy.show', $member->user->wallet_address) }}">
                    <img src="{{ $member->present()->genealogyImage() }}"
                         alt="{{ $member->user->wallet_address }}"
                         style="background-color: {{ $member->present()->genealogyImageBackground() }};">
                </a>
        ...
        <p class="name">
            @if(Agent::isMobile())
                <span>
                    <a href="{{ route('admin.genealogy.show', $member->user->wallet_address) }}">{{ walletAddress($member->user->wallet_address) }}</a>
                </span>
            @else
