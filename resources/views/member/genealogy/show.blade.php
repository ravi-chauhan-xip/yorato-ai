...
@section('title') Genealogy Tree @endsection
...
{{--                    </div>--}}
                ...
                <div class="row miniReports">
                    <div class="col-md-3 col-6">
                        <h6 class="font-weight-600">DIRECT LEFT : {{ $member->sponsored_left }}</h6>
                        <h6 class="font-weight-600">LEFT TOP UP : {{ toHumanReadable($member->left_bv) }}</h6>
                        <h6 class="font-weight-600">LEFT TOP UP POWER : {{ toHumanReadable($member->left_power) }}</h6>
                        <h6 class="font-weight-600">LEFT STAKING : {{ toHumanReadable($member->left_stake_bv) }}</h6>
                        <h6 class="font-weight-600">LEFT STAKING POWER : {{ toHumanReadable($member->left_stake_power) }}</h6>
                        <h6 class="font-weight-600">TOTAL LEFT : {{ toHumanReadable($member->left_count) }}</h6>
                    </div>
                    <div class="offset-md-6 col-md-3 col-6 text-right">
                        <h6 class="font-weight-600">DIRECT RIGHT : {{ $member->sponsored_right }}</h6>
                        <h6 class="font-weight-600">RIGHT TOP UP : {{ toHumanReadable($member->right_bv) }}</h6>
                        <h6 class="font-weight-600">RIGHT TOP UP POWER: {{ toHumanReadable($member->right_power) }}</h6>
                        <h6 class="font-weight-600">RIGHT STAKING : {{ toHumanReadable($member->right_stake_bv) }}</h6>
                        <h6 class="font-weight-600">RIGHT STAKING POWER: {{ toHumanReadable($member->right_stake_power) }}</h6>
                        <h6 class="font-weight-600">TOTAL RIGHT : {{ toHumanReadable($member->right_count) }}</h6>
                    </div>
                </div>
                <section class="management-hierarchy">
                    <div class="hv-container">
                        <div class="hv-wrapper">
                            @include('member.genealogy.member', ['member' => $member, 'level' => 0, 'side' => $side])
                        </div>
                    </div>
                </section>
            </div>
    ...
    <script>
        function goToMember() {
            ...
            if (trackingCode.length) {
                window.location = '{{ route('user.genealogy.show') }}/' + trackingCode;
            }
        }
